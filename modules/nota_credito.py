...
class GenerarNotaCredito(models.TransientModel):
    _name = "generar.nota.credito"

    def generar_nota_credito(self, factura, correlativo):
        if not factura.reversed_entry_id:
            raise ValidationError(
                "La nota de crédito debe ser creada a base de una factura existente"
            )
        ...
        correo_emisor = factura.company_id.email if factura.company_id.email else ""
        ...
        if factura.fiscal_position_id.frases_fel_id:
            for frase in factura.fiscal_position_id.frases_fel_id:
                if frase.name != "Exportaciones":
                    ...
                    continue
        if factura.reversed_entry_id.exento_iva:
        ...
        referencias_nota = ET.SubElement(
            ...
            {
                "xmlns:cno": "http://www.sat.gob.gt/face2/ComplementoReferenciaNota/0.1.0",
                "FechaEmisionDocumentoOrigen": f"{factura.reversed_entry_id.invoice_date}",
                "MotivoAjuste": f"{factura.ref}",
                "NumeroAutorizacionDocumentoOrigen": f"{factura.reversed_entry_id.numero_autorizacion}",
                "NumeroDocumentoOrigen": f"{factura.reversed_entry_id.numero_dte}",
                "SerieDocumentoOrigen": f"{factura.reversed_entry_id.serie}",
                "Version": "1",
            },
        )
        ...
        # EXPORTACIÓN
        if factura.reversed_entry_id.fiscal_position_id.name == "Exportación":
