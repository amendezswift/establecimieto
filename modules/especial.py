...
class GenerarEspecial(models.TransientModel):
    _name = "generar.especial"

    def generar_especial(self, factura, correlativo):
        ...
        frase_cinco = ET.SubElement(
            dte_frases, "dte:Frase", {"TipoFrase": "5", "CodigoEscenario": "1"}
        )
        ...
        for line in factura.invoice_line_ids:
            ...
            for imp in line.tax_ids:
                if imp.nombre_corto == "BEBIDAS ALCOHOLICAS" and imp.price_include:
                    ...
                    impuestos_incluidos_precio += (
                        (imp.amount / 100)
                        * line.product_uom_id.ratio
                        * line.product_id.contenido_neto
                    )
            ...
            dte_precio = ET.SubElement(dte_item, "dte:Precio").text = (
                f"{round((precio_unitario_sat * line.quantity), 4)}"
            )
            ...
            for impuesto in line.tax_ids:
                if impuesto.name not in [
                    "Retención IVA factura especial",
                    "Retención ISR factura especial",
                ]:
                    ...
                    # SI NO ES ALCOHOLICA
                        ...
                        monto_gravable = ET.SubElement(
                            dte_impuesto, "dte:MontoGravable"
                        ).text = f"{calculo_monto_gravable}"
                        importe = impuesto.amount / 100
                        monto = round((calculo_monto_gravable * importe), 6)
                        monto_impuesto = ET.SubElement(
                            dte_impuesto, "dte:MontoImpuesto"
                        ).text = f"{monto}"
                    ...
                    impuestos_encontrados[nombre] = (
                        impuestos_encontrados.get(nombre, 0) + monto
                    )
                else:
                    continue
            ...
            monto_isr_ = round((monto_total_ * 0.05), 2)
            monto_iva_ = round((monto_total_ * 0.12), 2)
            gran_total_ = round((line.price_total + monto_isr_ + monto_iva_), 6)
            amount_taxesless += gran_total_
            dte_total = ET.SubElement(dte_item, "dte:Total").text = f"{round(gran_total_, 6)}"
        ...
        gran_total = ET.SubElement(dte_totales, "dte:GranTotal").text = f"{amount_taxesless}"
        ...
        retenciones_especial = ET.SubElement(
            dte_complemento,
        ...
        monto_total = factura.amount_untaxed
        monto_isr = round((monto_total * 0.05), 6)
        monto_iva = round((monto_total * 0.12), 6)

        retencion_isr = ET.SubElement(retenciones_especial, "cfe:RetencionISR").text = (
            f"{monto_isr}"
        )
        retencion_iva = ET.SubElement(retenciones_especial, "cfe:RetencionIVA").text = (
            f"{monto_iva}"
        )
        total_sin_retenciones = ET.SubElement(
            retenciones_especial, "cfe:TotalMenosRetenciones"
        ).text = f"{round(factura.amount_total, 6)}"
